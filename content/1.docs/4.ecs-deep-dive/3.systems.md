...
::card-group
  :::card{icon="i-lucide-filter" title="Запросы и фильтры" to="/4.ecs-deep-dive/4.queries-filters"}
  Изучите, как эффективно запрашивать сущности с определенными компонентами.
  :::
  :::card{icon="i-lucide-globe" title="Веб-интерфейсы" to="/5.web-interfaces/1.overview"}
